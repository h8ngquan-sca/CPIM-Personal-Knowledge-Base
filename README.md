# 📦 APICS CPIM - Personal Knowledge Repository

Welcome to my personal notes for Supply Chain Planning! 🎯

This repository documents my structured learning journey as I prepare for the **APICS CPIM (Certified in Planning and Inventory Management)** exam. Instead of just highlighting textbooks, I use this space to synthesize concepts, build frameworks, and organize my knowledge systematically.


## ⚠️ Disclaimer
* **Current Status:** I am currently a CPIM Candidate preparing for the exam (Target: [Month, Year]). I do not yet hold the CPIM certification.
* **Copyright Notice:** This repository contains my *personal study notes, summaries, and mindmaps* written in my own words. It DOES NOT contain official APICS/ASCM proprietary materials, test banks, or verbatim copies of the learning system. 


## 🏗️ Learning System

**MODULE 1: Supply Chains and Strategy**

**MODULE 2: Sales & Operations Planning**

**MODULE 3: Demand**

**MODULE 4: Supply**

**MODULE 5: Detailed Scheduless**

**MODULE 6: Inventory**

**MODULE 7: Distribution**

**MODULE 8: Quality, Technology, and Continuous Improvement**



> *"Failing to plan is planning to fail." - Alan Lakein*
